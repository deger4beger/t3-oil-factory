...
export const statisticsRouter = createProtectedRouter().query("getTotal", {
	input: z.object({
	...
	async resolve({ ctx, input }) {
		...
		const purchasesPrice = purchases.reduce(
			(acc, purchase) => (acc += purchase.price),
			0
		);
		const salesPrice = sales.reduce((acc, sale) => (acc += sale.price), 0);

		const operationsByDate = (
			await ctx.prisma.operation.groupBy({
				where: {
					createdAt: {
						gte: input.from,
						lte: input.to,
			...
			.reduce(
				(acc, operations) => {
					const existed = acc.find(
						(operation) => operation.date === operations.date
					);
					if (!existed)
						acc.push({
							закупки: 0,
