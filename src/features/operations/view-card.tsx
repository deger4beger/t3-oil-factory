...
const ViewCard = ({
	...
	const { mutateAsync: update, isLoading: updateLoading } = trpc.useMutation(
		["operation.update"],
		{ onSuccess }
	);
	const { mutate: deleteOne, isLoading: deleteLoading } = trpc.useMutation(
		["operation.delete"],
		{ onSuccess }
	);
	...
	return (
		<div
			className={
				"flex flex-col justify-between text-zinc-200 border-4 border-zinc-700 p-4 m-2 flex-grow rounded-xl" +
				(operation === "SALE" ? " bg-zinc-800" : "")
			}
		>
			<div className="inline-flex font-semibold text-base mb-2 items-center justify-between">
				<div>
					Операция от {createdAt.toLocaleDateString()}{" "}
			...
			<div className={styles2}>
				<span className={styles}>Название:</span>
				{name}
			</div>
			<div className={styles2}>
				<span className={styles}>Автор:</span>
				{user.name}
			</div>
			<div className={styles2}>
				<span className={styles}>Цена:</span>
				{price}
			</div>
			<div className={styles2}>
				<span className={styles}>Количество:</span>
				{count}
			</div>
			<div className={styles2}>
				<span className={styles}>Общая цена:</span>
				{count * price}
			</div>
			<div className={styles2}>
				<span className={styles}>Дата оформления:</span>
